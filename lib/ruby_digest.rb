...
module RubyDigest
  # The semantic version of {RubyDigest}.
  ...
  module Instance
    ...
    #   object
    # @see RubyDigest.bubblebabble
    def bubblebabble
      RubyDigest.bubblebabble(digest)
    end
    ...
    # @return [RubyDigest::Class] a new, initialized copy of this digest object
    def new
      clone.reset
    end
  ...
  class Class
    ...
    # @see RubyDigest.bubblebabble
    def self.bubblebabble(string)
      RubyDigest.bubblebabble(digest(string))
    end
  ...
  class Base < Class
    ...
    # object to be initialized is a {RubyDigest::Base} object.
    # 
    # @param args [Array<Object>] the arguments to pass to `#initialize`
    # @return [RubyDigest::Base] the new digest object instance
    ...
    # 
    # @param source [RubyDigest::Base] the digest object being duplicated
    # @return [RubyDigest::Base] the duplicate digest object
  ...
  class SHA2 < Class
    ...
    def initialize(bit_length = 256)
      case bit_length
      when 256 then @sha2 = RubyDigest::SHA256.new
      else
    ...
    # 
    # @param source [RubyDigest::SHA2] the {SHA2} object being duplicated
    # @return [RubyDigest::SHA2] the duplicate {SHA2} object
    def initialize_copy(source)
      super
  ...
  class HMAC < Class
    ...
    # @param digest_class [RubyDigest::Base] the digest object for this {HMAC}
    ...
    #   the same base digest object as the source digest object.
    # 
    # @param source [RubyDigest::HMAC] the {HMAC} object being duplicated
    # @return [RubyDigest::HMAC] the duplicate {HMAC} object
    def initialize_copy(source)
      super
...
# Returns a {RubyDigest} subclass by the given `name`.
# 
# @param name [String, Symbol] the {RubyDigest} subclass to return
# @return [RubyDigest::Base] the requested {RubyDigest} subclass if it exists
# @raise [NameError] if the given subclass `name` does not exist
def RubyDigest(name)
  RubyDigest.const_get(name)
end

unless defined?(Digest)
  # Links the {Digest} constant to {RubyDigest}, allowing {RubyDigest} to
  # supply a pure-Ruby implementation of the MRI Ruby 'digest' standard
  # library.
  Digest = RubyDigest
end

alias :Digest :RubyDigest unless defined?(Digest())
